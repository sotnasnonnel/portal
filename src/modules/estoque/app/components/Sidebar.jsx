...
export default function Sidebar({ aberto = false, onFechar }) {
  ...
  const secoes = navSections({ operador });
  ...
  return (
    <>
      ...
      <div
        className={`estSb-overlay ${aberto ? 'is-visible' : ''}`}
        onClick={onFechar}
        aria-hidden="true"
      />

      <aside className={`estSb ${aberto ? 'is-open' : ''}`}>
        <Link
          to="/home"
          className="estSb-brand"
          title="Voltar ao início"
          aria-label="Voltar ao início"
          onClick={onFechar}
        >
          <span className="estSb-logo" aria-hidden="true"><Boxes size={20} /></span>
          <strong className="estSb-brandtext">Estoque</strong>
        </Link>

        <nav className="estSb-nav">
          <AppSwitcher currentKey="estoque" onNavigate={onFechar} />
          {secoes.map((sec) => (
            <div key={sec.label}>
              <div className="estSb-seclabel">{sec.label}</div>
              {sec.items.map((item) => (
                <Link
                  key={item.href}
                  to={item.href}
                  title={item.label}
                  className={`estSb-link ${pathname.startsWith(item.href) ? 'is-active' : ''}`}
                  onClick={onFechar}
                >
                  <item.Icon size={16} />
        ...
        <div className="estSb-footer">
          <AvatarUsuario className="estSb-avatar" iniciais={iniciais(nome, email)} />
          <div className="estSb-userinfo">
            <strong title={nome || email}>{nome || 'Usuário'}</strong>
            <span>{operador ? 'Almoxarifado' : 'Consulta'}</span>
          </div>
          <button className="estSb-logout" onClick={logout} title="Sair" type="button">
            <LogOut size={18} />
          </button>
        </div>
      </aside>
