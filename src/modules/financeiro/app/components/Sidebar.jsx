...
export default function Sidebar() {
  ...
  const canAbrir = ['coordenador', 'gestor'].includes(user?.perfil);
  ...
  return (
    <aside className="finSb">
      <Link to="/home" className="finSb-brand" title="Voltar ao início" aria-label="Voltar ao início">
      ...
      <nav className="finSb-nav">
        <AppSwitcher currentKey="financeiro" />
        {secoes.map((sec) => {
          if (!sec.group) {
            return (
              <div key={sec.label}>
      ...
      <div className="finSb-footer">
        <div className="finSb-avatar">{iniciais(nome, email)}</div>
